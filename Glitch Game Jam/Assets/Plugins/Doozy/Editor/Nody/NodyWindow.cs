...
namespace Doozy.Editor.Nody
{
    public class NodyWindow : FluidWindow<NodyWindow>
    {
        ...
        public static void CleaNodesLeavingOnlyNativeNodes()
        {
            FlowNodeViewExtensionGenerator.Run(true);
            NodyNodeSearchWindowGenerator.Run(true);
            UnityEditor.Compilation.CompilationPipeline.RequestScriptCompilation();
        }

        [RefreshData("Nody Node Search Window")]
        [MenuItem("Tools/Doozy/Refresh/Nody", priority = -450)]
        public static void Refresh()
        {
            FlowNodeViewExtensionGenerator.Run(false);
            NodyNodeSearchWindowGenerator.Run(false);
            UnityEditor.Compilation.CompilationPipeline.RequestScriptCompilation();
        }
